        ...
        local settings = gui:create("D", "Settings"); do
            ...
            local extra = settings:group "Extra"; do
                ...
                extra.ragdolls = extra:switch("Ragdolls", false, function(gear)
                    return {
                        gravity = gear:slider("Gravity", -200, 150, round(cvar.ragdoll_gravity_scale:float() * 100),
                            "%s%%"),
                        fraction = gear:slider("Fraction", 0, 100, round(cvar.ragdoll_friction_scale:float() * 100),
                            "%s%%"),
                    }
                end)
            end
    ...
    local visuals = {}; do
        ...
        local widgets = {}; do
            ...
            local logger = {}; do
                ...
                local allow_weapons = { "knife", "c4", "decoy", "flashbang", "hegrenade", "incgrenade", "molotov",
                    "inferno",
                    "smokegrenade" }
            ...
            function scope_overlay.paint()
                ...
                if alpha > .1 then
                    ...
                    if not exclude_lines:get "Right" then
                        render.rect_filled_fade(position + vec2_t(offset + 1, 0),
                            position + vec2_t(offset + length + 1, 1),
                    ...
                    if not exclude_lines:get "Down" then
                        render.rect_filled_fade(position + vec2_t(0, offset + 1),
                            position + vec2_t(1, offset + length + 1),
...
__bundle_register("engine/entity", function(require, _LOADED, __bundle_register, __bundle_modules)
    ...
    local set_model; do
        ...
        local sig = assert(
            find_pattern("client.dll", "40 53 48 83 ? ? 48 8B ? 4C 8B ? 48 8B ? ? ? ? ? 48 8D ? ? ? 48 8B"),
            "fnSetModel outdated")
        ...
        local gui_render = {}; do
            ...
            function gui_render:gear(item, pos, alpha, mouse)
                if item.__gear and item.__gear.items and #item.__gear.items > 0 then
                    ...
                    item.__gear.icon_pct = tweening:interp(item.__gear.icon_pct, item.__gear.opened and 1 or 0,
                        ANIM_SPEED)
            ...
            function gui_render:switch(item, pos, column_width, alpha, mouse, it_gear, gear)
                ...
                local text_color = color_t(.75, .75, .75, 1):lerp(color_t(.92, .92, .92, 1), item.pct):alpha_modulate(
                alpha)
            ...
            function gui_render:slider(item, pos, column_width, alpha, mouse, it_gear, gear)
                ...
                render.rect_filled(vec2_t(x + box_size.x + arf_width * .5 + 4, y + 1),
                    vec2_t(x + box_size.x + arf_width * .5 + 5, y + 1 + arf_height),
                    COLORS.TEXT:alpha_modulate(.5 * alpha))
                ...
                if item.pct > .08 then
                    ...
                    for i = 0x2F, 0x5A do
                        if c_input:is_key_clicked(i) then
                            ...
                            item.value = item.value:sub(1, item.cursor_pos - 1) ..
                            symbol .. item.value:sub(item.cursor_pos)
                ...
                if item.pct ~= 0 then
                    if item.interacting == 1 or item.interacting == 3 or item.interacting == 4 or item.interacting == 5 then
                        ...
                        do
                            ...
                            local hue_colors = { color_t(1, 0, 0, 1), color_t(1, 1, 0, 1), color_t(0, 1, 0, 1), color_t(
                                0, 1,
                            ...
                            for i = 1, #hue_colors - 1 do
                                ...
                                post_render.rect_filled_fade(p, p + vec2_t(width, height * t), hue_colors[i],
                                    hue_colors[i],
                        ...
                        do
                            ...
                            for i = 0, size.y do
                                ...
                                post_render.rect_filled_fade(pos + vec2_t(0, i), pos + vec2_t(size.y, i + 1), c2, c1, c1,
                                    c2);
                            end
                            ...
                            post_render.circle(vec2_t(pos.x + size.x * hsv[2], pos.y + size.y * (1 - hsv[3])), size_color,
                                18,
                                true, color);
                            post_render.circle(vec2_t(pos.x + size.x * hsv[2], pos.y + size.y * (1 - hsv[3])), size_color,
                                18,
                            ...
                            if i > 1 then
                                if in_bounds(pos, pos + measure - 2, mouse.pos) then
                                    ...
                                    if i == 2 and mouse.is_lmb then
                                        clipboard.set(string.format("#%02x%02x%02x%02x", r * 255, g * 255, b * 255,
                                                a * 255)
                                            :upper())
        ...
        function gui:paint()
            ...
            if alpha >= .1 then
                ...
                local this_tab = selected.tab; do
                    ...
                    render.rect_filled(vec2_t(x, y + offset_y),
                        vec2_t(x + tabs_info.size.x, y + offset_y + tabs_info.size.y),
                    ...
                    if this_tab ~= nil then
                            ...
                            for _, item in pairs(column.items) do
                                ...
                                if instanceof == switch then
                                    if item.offset_pct > 0.1 then
                                        ...
                                        local add, gear_data = gui_render:switch(item, vec2_t(x, y), column_width,
                                            item_alpha,
                                ...
                                if instanceof == slider then
                                    if item.offset_pct > 0.1 then
                                        local mouse_data = { is_lmb = is_lmb, is_left_button = is_left_button, pos =
                                        mouse }
                                        local add, gear_data = gui_render:slider(item, vec2_t(x, y), column_width,
                                            item_alpha,
                                            mouse_data, false, true)
                                        local path = f("%s.%s.%s", this_tab.name, column.__name, item.name)
                                ...
                                if instanceof == label then
                                    if item.offset_pct > 0.1 then
                                        local mouse_data = { is_lmb = is_lmb, pos = mouse }
                                        local add, gear_data = gui_render:label(item, vec2_t(x, y), column_width,
                                            item_alpha,
                                            mouse_data, false, true)
                                        local path = f("%s.%s.%s", this_tab.name, column.__name, item.name)
                                ...
                                if instanceof == input then
                                    if item.offset_pct > 0.1 then
                                        local mouse_data = { is_lmb = is_lmb, pos = mouse }
                                        local add = gui_render:input(item, vec2_t(x, y), column_width, item_alpha,
                                            mouse_data,
                                            false)
                        ...
                        for _, popup_data in pairs(gear_popups) do
                            ...
                            if pct > 0.1 then
                                ...
                                render.rect(vec2_t(popup_x, popup_y),
                                    vec2_t(popup_x + popup_width, popup_y + popup_height),
                                ...
                                for _, item in pairs(popup_item.__gear.items) do
                                    ...
                                    item.offset_pct = tweening:interp(item.offset_pct, item:visibility() and pct or 0,
                                        .05)
                                    ...
                                    if instanceof == color_picker then
                                        if item.offset_pct > 0.1 then
                                            ...
                                            local add = gui_render:color_picker(item, vec2_t(x, y), popup_width,
                                                item_alpha,
                                                mouse_data, true)
                ...
                local avatar_pos = position +
                vec2_t(avatar_size + PADDING * 1.5, size.current.y - avatar_size - PADDING * 2)
...
__bundle_register("render/windows", function(require, _LOADED, __bundle_register, __bundle_modules)
    ...
    local windows = {}; do
        ...
        local c_draggable = {}; do
            ...
            function c_draggable:update(from_ui)
                ...
                for i, rule in ipairs(self.rules) do
                    ...
                    self.animations[i] = tweening:interp(self.animations[i],
                        (dragged_window == self and not control_down) and (align_dist < 10 and 120 / 255 or 60 / 255) or
                        0,
                        0.05)
            ...
            function c_draggable:render()
                ...
                self.animations.hover = tweening:interp(self.animations.hover,
                    self.is_active and (self.is_hovered and (c_input:is_key_pressed(0x01) and 0.4 or 0.2) or 0) or 0,
                    0.05)
        ...
        register_callback("paint", function()
            ...
            background_alpha = tweening:interp(background_alpha,
                (dragged_window ~= nil and windows.background) and 1 or 0,
                0.075)
    ...
    local http = {}; do
        ...
        local function get_status_code(request)
            ...
            if winhttp.WinHttpQueryHeaders(request, query_flags, nil, status_buffer, status_buffer_size, nil) ~= 0 then
                ...
                local error_msg = "WinHttpQueryHeaders failed to retrieve status code. Error: " ..
                tostring(C.GetLastError());
                return -1, error_msg;
            end;
        end;
    ...
    local events, debug_enabled = {}, _DEBUG; do
        ...
        local fnCreateMove, GetUserCmd; do
            ...
            local fnGetUserCmd = ffi.cast("CUserCmd*(__fastcall*)(void*, int)",
                assert(opcode_scan("client.dll", "40 53 48 83 EC ?? 8B DA E8 ?? ?? ?? ?? 4C 8B C0"),
                    "fnGetUserCmd: outdated"))
